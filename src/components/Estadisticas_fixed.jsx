...
export default function Estadisticas() {
  ...
  const [filterType, setFilterType] = useState('week'); // 'week' o 'dateRange'
  ...
  const testFirebaseConnection = async () => {
    try {
      ...
      if (testSnapshot.size > 0) {
        console.log('✅ Conexión exitosa! Primeros 3 documentos:');
        testSnapshot.docs.slice(0, 3).forEach((doc, index) => {
          console.log(`Documento ${index + 1}:`, doc.id, doc.data());
        });
        alert(`✅ Conexión exitosa! Se encontraron ${testSnapshot.size} documentos en feedingRecords.`);
      } else {
